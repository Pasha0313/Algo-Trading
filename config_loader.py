...
def get_control_settings(config):
    ...
    return Perform_Testing, Print_Data, Perform_Trading
...
def prepare_data_from_config(config):
    ...
    return start_date, end_date, symbol, bar_length, leverage, strategy, tc, test_days
...
def prepare_trade_from_config(config):
    ...
    trade_data = [
        ...
        ["loading from Date", loading_from_date.strftime('%Y-%m-%d %H:%M:%S')],
        ["Trade Start Date", Today.strftime('%Y-%m-%d %H:%M:%S')],
        ["Trade Stop Date", stop_trade_date.strftime('%Y-%m-%d %H:%M:%S')]        
    ]
